...
const SearchNFT: React.FC = () => {
  ...
  const searchNFTs = async () => {
    ...
    try {
      ...
      const processedResponse = Array.isArray(response) ? response[0] : response;

      if (processedResponse && processedResponse.length > 0) {
        const nftPromises = processedResponse.map(async (nftId: any) =>
          fetchNFTDataUtil(nftId[0], account?.address, client)
        );
              ...
              >
                <div style={{ display: "flex", flexDirection: "column" }}>
                  ...
                  <Space style={{ width: "100%", justifyContent: "space-between", alignItems: "center" }}>
                    <Paragraph style={{ margin: '0px' }}></Paragraph>
                    <Text
                      style={{ fontSize: '12px', fontWeight: 'bold', margin: '5px' }}
                      type="secondary"
                    >
                      <span style={{ color: rarityColors[nft.rarity] }}>
